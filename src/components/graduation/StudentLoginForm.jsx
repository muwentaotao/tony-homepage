...
export default function StudentLoginForm({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/graduation/student/login', {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(data.error || '姓名或密码错误');
        return;
      }

      sessionStorage.setItem('graduation_token', data.token);
      onLogin(data.token, data.isFirstLogin);
    } catch (err) {
      setError('登录失败，请稍后重试');
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-5 py-12 relative">
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-tony-muted">2026 届毕业留言</p>
        </div>

        <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-8 space-y-6">
          <div>
            <label className="block text-sm text-tony-muted mb-2">姓名</label>
            <input
              type="text"
              value={name}
          ...
          <p className="text-center text-xs text-tony-muted/70">
            这是只属于你的留言，请珍惜保存
          </p>
        </form>
      </div>
